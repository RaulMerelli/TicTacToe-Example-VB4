...
Private Sub Mark(x As Integer, y As Integer)
    ...
    Controls("PIC" & CStr(x) & CStr(y)).Picture = LoadPicture("RES\" & 100 + Turno & ".BMP")
    Controls("PIC" & CStr(x) & CStr(y)).Visible = True
    Turno = IIf(Turno = 1, 2, 1)
End Sub

Private Sub Reset()

    If Not (Dir("RES\101.BMP") <> "" And Dir("RES\102.BMP") <> "") Then
        If vbOK = MsgBox("Impossibile avviare:" & vbCrLf & "Controllare che i file 101.bmp e 102.bmp siano presenti nella cartella res.", vbOKOnly, "Errore") Then
            Unload Me
            Exit Sub
        End If
    End If
    ...
    For y_loc = 0 To 2
        For x_loc = 0 To 2
            ...
            Controls("PIC" & CStr(x_loc) & CStr(y_loc)).Visible = False
        Next
    Next
End Sub
